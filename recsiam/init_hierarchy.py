...
def build_jl_transform(samples, jl_dim, jl_seed=0):
    """Build a Johnson–Lindenstrauss projection matrix sized to the dataset's
    embeddings, probing ``samples[0]`` for the input dimension. Returns None
    when ``jl_dim`` is None or there are no samples."""
    if jl_dim is None or len(samples) == 0:
        return None
    probe = _load_embedding(samples[0]["path"])
    in_dim = probe.shape[-1]
    jl_transform = _make_jl_projection(in_dim, jl_dim, seed=jl_seed)
    logging.info("JL projection: %d → %d dims (seed=%d)", in_dim, jl_dim, jl_seed)
    return jl_transform
