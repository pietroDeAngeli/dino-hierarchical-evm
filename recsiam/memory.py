...
class ObjectsMemory(object):
    ...
    def _query_global_faiss(self, pos_X, pos_idxs):
        ...
        applied on the random path (EVM._max_neg_cap, default multiplier
        10). Without this, k scales with neg_size * n_neg_available and
        can reach tens of thousands on large nodes, even though
        _weibull_fit only ever consumes the `tailsize` nearest negatives
        per positive -- the rest would just cost extra FAISS query time
        and memory for no benefit.

        Same-class vectors are excluded natively at search time via a
        FAISS IDSelector, so exactly k neighbours are fetched per query
        (no k + n_same over-fetch, no Python-side masking).

        Queries are batched to keep peak memory bounded (batch × k × 12 B).
        ...
        # Default multiplier mirrors EVM.__init__ (max_neg_multiplier=10):
        # leaving k uncapped when the config omits the key made the FAISS
        # path fetch (a large multiple of) the whole index per query.
        max_neg_multiplier = self.evm_args.get("max_neg_multiplier", 10)
        if max_neg_multiplier is not None:
            k_cap = max(1, int(max_neg_multiplier) * tailsize)
            k = min(k_frac, k_cap)
        else:
            k = k_frac
        k = min(k, n_neg_available)

        # Exclude same-class vectors inside FAISS instead of over-fetching
        # k + n_same and masking in Python. `sel` must stay referenced for
        # as long as `params` is used.
        sel = _faiss.IDSelectorNot(
            _faiss.IDSelectorBatch(np.asarray(pos_idxs, dtype=np.int64))
        )
        if isinstance(self._faiss_index, _faiss.IndexIVF):
            # nprobe must be repeated here: SearchParametersIVF overrides
            # the index-level value (and would default to 1 if omitted).
            params = _faiss.SearchParametersIVF(
                sel=sel, nprobe=self._faiss_index.nprobe
            )
        else:
            params = _faiss.SearchParameters(sel=sel)
        ...
        for qi in range(0, len(pos_X), query_batch):
            ...
            D_sq, I = self._faiss_index.search(batch, k, params=params)
            # Rows with fewer than k reachable negatives have trailing
            # I == -1 slots: pad them with the furthest valid distance
            # (or 0 if the row found none).
            deficient = np.nonzero(I[:, -1] < 0)[0]
            for li in deficient:
                have = int((I[li] >= 0).sum())
                pad_val = float(D_sq[li, have - 1]) if have > 0 else 0.0
                D_sq[li, have:] = pad_val
            np.sqrt(np.maximum(D_sq, 0.0, out=D_sq), out=D_sq)
            result[qi:qi + len(batch)] = D_sq
