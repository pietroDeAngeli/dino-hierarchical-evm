...
def set_cover_stream(X_label, weibulls, cover_threshold, keep_ind,
                     covered_points, cdist_fn, block_rows):
    """Same computation as set_cover, but the pairwise distance/probability
    matrix is produced in row blocks and thresholded on the fly, so only
    block_rows × n floats exist at any time instead of the dense n × n
    matrix. The retained coverage subsets are identical (same pairwise
    probabilities, same threshold), and the same greedy solver runs on them.
    """
    n = len(X_label)
    universe = range(n)
    subsets = [()] * n
    for start in range(0, n, block_rows):
        ...
        d_block = cdist_fn(X_label[start:stop], X_label)
        probs = np.array(list(map(_weibull_eval,
                                  zip(d_block, weibulls[start:stop]))))
        probs[np.arange(stop - start), np.arange(start, stop)] = 1.0
        rr, cc = np.where(probs >= cover_threshold)
        # np.where returns rr sorted: split cc into per-row groups
        split_at = np.searchsorted(rr, np.arange(1, stop - start))
        for li, cols in enumerate(np.split(cc, split_at)):
            subsets[start + li] = tuple(cols)
    return set_cover_greedy(universe, subsets, keep_ind, covered_points)
...
class EVM():
    ...
    def _compute_neg_d_faiss(self, X, neg_X):
        ...
        neg_size is a fraction in (0, 1]: 1.0 uses all negatives.
        k is capped like the random path (_max_neg_cap) and by the number
        of available negatives (topk would raise for k > len(neg_X))."""
        k = max(1, round(self.neg_size * len(neg_X)))
        k = min(k, self._max_neg_cap(), len(neg_X))
    ...
    def _stream_block_rows(self, n_cols, target_bytes=256 * 1024 * 1024):
        """Rows per streamed distance block so a block stays ~target_bytes
        of float64 (peak memory = block_rows × n_cols × 8 B)."""
        return max(32, int(target_bytes // (8 * max(1, n_cols))))
    ...
    def fit(self, X, y, neg_X=_EMPTY, neg_d=None):
        ...
        if neg_d is not None:
            assert n_new == neg_d.shape[0]
            n_neg = neg_d.shape[1]
        else:
            n_neg = len(neg_X)
        labels = empty_cat((new_y, np.tile(None, n_neg).astype(object)))
        n_cols = n_old + n_new + n_neg

        # The Weibull fit only consumes, per NEW point, its distances to
        # [old EVs | new points | negatives]; the old×old block of the
        # former dense d_mat was used exclusively by the reduce step (now
        # streamed inside reduce_model as well). Producing the new rows in
        # blocks keeps peak memory at block × n_cols instead of the
        # (n_old + n_new)² matrix that caused OOM on large nodes.
        block = self._stream_block_rows(n_cols)
        weibulls = []
        with timed("weibull_fit_loop", n_rows=n_new):
            for start in range(0, n_new, block):
                stop = min(start + block, n_new)
                parts = []
                if n_old > 0:
                    parts.append(self._cdist(X[start:stop], self.X))
                parts.append(self._cdist(X[start:stop], X))
                if neg_d is not None:
                    parts.append(neg_d[start:stop].astype(np.float64))
                elif n_neg > 0:
                    parts.append(self._cdist(X[start:stop], neg_X))
                rows = parts[0] if len(parts) == 1 else np.concatenate(parts, axis=1)
                args = zip((self.margin_scale * rows[i] for i in range(stop - start)),
                           range(n_old + start, n_old + stop),
                           it.repeat(labels),
                           it.repeat(self.tailsize))
                weibulls.extend(map(_weibull_fit, args))
        weibulls = np.array(weibulls, dtype=object)

        self.weibulls = empty_cat((self.weibulls, weibulls))
        self.X = new_X
        self.y = new_y
        self.update_labels()
        ...
        if self.reduce:
            with timed("reduce_model", n_total=len(self.X), n_added=len(X)):
                self.reduce_model(indices=slice(len(self.X) - len(X),
                                                len(self.X), None))
        else:
    ...
    def reduce_model(self, indices=None):
        """
        Model reduction routine. Calls off to set cover.

        Same-label pairwise distances are computed in streamed row blocks
        inside set_cover_stream instead of taking a dense precomputed
        d_mat: peak memory is block × n_label per label instead of the
        full quadratic matrix.
        """
        ...
        for ulabel in self.labels:
            ...
            X_label = self.X[ind]
            keep_ind, num_cover = set_cover_stream(X_label,
                                                   self.weibulls[ind],
                                                   self.cover_threshold,
                                                   np.arange(keep_ind.size),
                                                   self.covered_points[keep_ind],
                                                   self._cdist,
                                                   self._stream_block_rows(len(ind)))
