...
def _load_split_embeddings(samples: Sequence[dict], jl_transform=None):
    """Load and mean-pool embeddings for a list of samples.

    If ``jl_transform`` is given, each embedding is projected with the
    Johnson–Lindenstrauss matrix (in_dim, out_dim) before mean-pooling.
...
def evaluate_logistic_head(clf: LogisticRegression, le: LabelEncoder, test_samples: Sequence[dict], gt_tree=None, jl_transform=None):
...
def main(cmdline):
    ...
    if cmdline.eval_test:
        metrics = evaluate_logistic_head(clf, le, test_samples, gt_tree=gt_tree, jl_transform=jl_transform)
