...
def compute_eval_metrics(true_labels, pred_labels, tree=None, gt_tree=None):
    ...
    if h_cache:
        ...
        for t_lab, p_lab in zip(true_labels, pred_labels):
            ...
            # Path correctness: pred and true lie on the same root->leaf path,
            # i.e. pred == true, pred is a proper ancestor of true, OR pred is a
            # proper descendant of true (true is an ancestor of pred).
            is_path_correct = float(
                is_h_correct or t_lab in ancestors_map.get(p_lab, frozenset())
            )
            correct_path_list.append(is_path_correct)
        ...
        if correct_H_list:
            ...
            correct_path_arr = np.array(correct_path_list, dtype=float)
            coverage_arr = np.array(coverage_list, dtype=float)

            hierarchical_accuracy = float(np.mean(correct_H_arr))
            path_accuracy = float(np.mean(correct_path_arr))
