...
class _ImageDataset(torch.utils.data.Dataset):
    """Loads images from disk; returns (numpy_array, path_str) or (None, path_str) on error."""
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if (len(img_array.shape) != 3 or img_array.shape[-1] != 3
                    or img_array.shape[0] < 5 or img_array.shape[1] < 5):
                return None, str(img_path)
            return img_array, str(img_path)
        except Exception:
            return None, str(img_path)


def _collate_fn(batch):
    valid = [(arr, path) for arr, path in batch if arr is not None]
    invalid_paths = [path for arr, path in batch if arr is None]
    if not valid:
        return [], [], invalid_paths
    arrays, paths = zip(*valid)
    return list(arrays), list(paths), invalid_paths


def embed_dataset(dataset_dir, model_name, batch_size, keep_images, descriptor_path=None, num_workers=DEFAULT_NUM_WORKERS):
    ...
    model = AutoModel.from_pretrained(model_name, token=HF_TOKEN, dtype=torch.bfloat16).to(device)
    model.eval()
    ...
    with tqdm(total=len(image_paths), desc="Embedding images") as pbar:
        for arrays, paths, invalid_paths in loader:
            for p in invalid_paths:
                print(f"  [WARN] Skipping corrupted/tiny image: {p}")
            pbar.update(len(invalid_paths))

            if not arrays:
                continue

            inputs = processor(images=arrays, return_tensors="pt").to(device)
            with torch.no_grad():
                outputs = model(**inputs)
            cls_embeddings = outputs.last_hidden_state[:, 0, :].float().cpu().numpy()

            for path_str, emb in zip(paths, cls_embeddings):
                path = Path(path_str)
...
def main():
    ...
    parser.add_argument("--num-workers", type=int, default=DEFAULT_NUM_WORKERS,
                        help=f"Number of DataLoader worker processes (default: {DEFAULT_NUM_WORKERS})")
    args = parser.parse_args()

    embed_dataset(args.dataset_dir, args.model, args.batch_size, args.keep_images, args.descriptor, args.num_workers)
